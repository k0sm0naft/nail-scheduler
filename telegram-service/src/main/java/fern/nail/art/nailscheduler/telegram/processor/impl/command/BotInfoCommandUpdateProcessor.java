...
public class BotInfoCommandUpdateProcessor extends AbstractCommandUpdateProcessor {
    private final MessageService messageService;
    private final UserService userService;

    @Override
    protected Command command() {
        return Command.BOT_INFO;
    }

    @Override
    public void process(Update update, User user) {
        clearPreviousMenu(user, messageService);
        Integer messageId = messageService.sendTextAndGetId(user, "Here will be bot info");
